  ...
  move = (dir: PLAYER_MOVE) => {
    switch (dir) {
      case PLAYER_MOVE.LEFT:
        if (this.x > 0) this.x -= PLAYER_WIDTH;
        this.domElement.setAttribute("class", "playerL");
        break;
      case PLAYER_MOVE.RIGHT:
        if (this.x < GAME_WIDTH - PLAYER_WIDTH) this.x += PLAYER_WIDTH;
        this.domElement.setAttribute("class", "playerR");
        break;
      case PLAYER_MOVE.UP:
        if (this.y > 0) this.y -= PLAYER_HEIGHT;
        break;
      case PLAYER_MOVE.DOWN:
        if (this.y < GAME_HEIGHT - PLAYER_HEIGHT) this.y += PLAYER_HEIGHT;
        break;
    }
