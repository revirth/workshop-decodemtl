        ...
        this.gameloop = () => {
            ...
            this.enemies.forEach(enemy => {
                ...
                if (this.hitTest(enemy.domElement, this.player.domElement, 35)) {
                    enemy.explose();
                    PLAYER_LIFE--;
                    consoleD("TCL: Engine -> gameloop -> hitTest", enemy.spot, "life :", PLAYER_LIFE);
                }
            });
            this.enemies = this.enemies.filter(enemy => !enemy.destroyed);
            while (this.enemies.length < ENEMY_NUMBER)
                this.enemies.push(new Enemy(this.root, this.nextEnemySpot(this.enemies), 0.05 + Math.random() * 0.1, Math.random() >= 0.5 // move left or right?
                ));
            consoleD("TCL: Engine -> gameloop -> Speed", (Math.random() + 0.3) / (ENEMY_NUMBER - PLAYER_LEVEL));
            //#region SCORE
